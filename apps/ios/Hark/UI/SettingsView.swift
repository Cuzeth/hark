...
struct SettingsView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                Text("Critical alerts sound even when this iPhone is muted or in a Focus. They are meant for genuine emergencies only and are not a substitute for contacting local emergency services.")
                    .font(.system(size: 11)).foregroundStyle(HarkTheme.muted).lineSpacing(3).padding(.vertical, 8)
    ...
    private func settingsRow(icon: String, label: String, value: String, opensSettings: Bool = false) -> some View {
        Button {
            ...
            HStack(spacing: 10) {
                Image(systemName: icon).font(.system(size: 14)).foregroundStyle(HarkTheme.accent)
                    .frame(width: 30, height: 30).background(HarkTheme.accentSoft, in: Circle())
                Text(label).font(.system(size: 13, weight: .medium)).foregroundStyle(HarkTheme.ink)
                Spacer()
                Text(value).font(.system(size: 12)).foregroundStyle(HarkTheme.soft).lineLimit(1)
                if opensSettings { Image(systemName: "chevron.right").font(.system(size: 10, weight: .semibold)).foregroundStyle(HarkTheme.soft) }
            }
            .frame(minHeight: 58)
            .overlay(alignment: .top) { Rectangle().fill(HarkTheme.line).frame(height: 0.5) }
        }
