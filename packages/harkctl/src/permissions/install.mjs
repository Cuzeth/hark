...
const LABEL = "dev.abdeen.hark-permission-bridge";
const STATUS_MESSAGE = "Waiting for Hark approval...";
...
async function atomicWrite(path, content, expected) {
  ...
  const temporary = `${path}.hark-${process.pid}-${Date.now()}`;
  try {
...
function isCodexHandler(handler) {
  return (
    ...
    handler.command.includes("harkctl") &&
...
export async function installCodex({ home = homedir(), entrypoint }) {
  ...
  const config = await readJson(path, { description: "Hark permission approvals.", hooks: {} });
  config.value.description ??= "Hark permission approvals.";
  config.value.hooks ??= {};
  ...
  return {
    ...
    path,
    warnings,
    next: "Open Codex /hooks and trust the Hark hook.",
  };
}
...
export function launchAgent({ home = homedir(), entrypoint }) {
  const stateDirectory = join(home, "Library", "Application Support", "Hark", "permission-bridge");
  const logDirectory = join(home, "Library", "Logs", "Hark");
...
const harkctl = ${JSON.stringify(entrypoint)};
const relevant = new Set(["server.connected", "permission.asked", "permission.updated"]);

export const HarkPermissionsV1Plugin = async ({ serverUrl, directory }) => ({
  event: async ({ event }) => {
    if (!relevant.has(event.type)) return;
    const child = spawn(harkctl, ["permissions", "opencode-v1-event"], {
      detached: true,
...
export async function installOpenCode({
  ...
  const pluginPath = join(home, ".config", "opencode", "plugin", "hark-permissions-v1.js");
  const stateDirectory = join(home, "Library", "Application Support", "Hark", "permission-bridge");
  const logDirectory = join(home, "Library", "Logs", "Hark");
...
export async function installationStatus({ home = homedir() } = {}) {
  ...
  const openCodeV1Path = join(home, ".config", "opencode", "plugin", "hark-permissions-v1.js");
  return {
