...
async function reply(serverUrl, directory, permission, decision) {
  try {
    await request(serverUrl, `/permission/${encodeURIComponent(permission.id)}/reply`, directory, {
      ...
      body: JSON.stringify({
        reply: decision,
        ...(decision === "reject" ? { message: "Denied through Hark." } : {}),
      }),
    });
  } catch (error) {
...
function startRequest(serverUrl, directory, request, ask = askHarkPermission) {
...
async function reconcile(serverUrl, directory, ask) {
  ...
  await Promise.all(pending.map((request) => startRequest(serverUrl, directory, request, ask)));
}

export async function handleOpenCodeV1Event(
  { serverUrl, directory, event },
  ask = askHarkPermission,
) {
  try {
