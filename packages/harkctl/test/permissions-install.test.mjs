...
async function temporaryHome() {
  return mkdtemp(join(tmpdir(), "hark-permissions-"));
}
...
test("Claude install is additive, idempotent, and surgically reversible", async () => {
  ...
  await installClaude({ home, entrypoint: "/opt/hark/harkctl.mjs" });
  await installClaude({ home, entrypoint: "/opt/hark/harkctl.mjs" });
...
test("Codex install preserves existing hooks and removes only Hark", async () => {
  const home = await temporaryHome();
  ...
  const installed = await installCodex({ home, entrypoint: "/opt/hark/harkctl.mjs" });
...
test("installers reject destructive JSON shapes without changing the file", async () => {
  ...
  await assert.rejects(
    installClaude({ home, entrypoint: "/opt/hark/harkctl.mjs" }),
